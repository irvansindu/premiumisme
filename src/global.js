...
function setupGlobalAnimations() {
  ...
  const revealSelectors = '.product-card, .article-card, .warranty-block, .accordion-item, .section-title, .banner-slider, .category-tabs, .footer-col, .hero-content, .hero-visual';
  document.querySelectorAll(revealSelectors).forEach((el, index) => {
    el.classList.add('reveal-up');
    
    // Optional stagger effect for adjacent siblings like cards in a grid
    if (el.classList.contains('product-card') || el.classList.contains('article-card')) {
       // Reset stagger purely based on visual rendering if needed, 
       // but DOM order index stagger works fine for simple grids
